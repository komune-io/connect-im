...
class F2Auth {
    ...
    companion object Plugin: HttpClientPlugin<io.komune.im.commons.F2Auth, io.komune.im.commons.F2Auth> {
        private var lastBearerTokens: BearerTokens? = null

        override val key: AttributeKey<io.komune.im.commons.F2Auth> = AttributeKey("F2Auth")

        override fun prepare(block: io.komune.im.commons.F2Auth.() -> Unit): io.komune.im.commons.F2Auth {
            return io.komune.im.commons.F2Auth().apply {
                block()
                authPlugin = prepareAuth()
            }
        }

        override fun install(plugin: io.komune.im.commons.F2Auth, scope: HttpClient) {
            Auth.install(plugin.authPlugin, scope)
        }

        private fun io.komune.im.commons.F2Auth.prepareAuth() = Auth.prepare {
            bearer {
                loadTokens {
                    io.komune.im.commons.F2Auth.Plugin.lastBearerTokens
                }
                refreshTokens {
                    val refreshTokenInfo: io.komune.im.commons.F2Auth.TokenInfo = client.post {
                    ...
                    io.komune.im.commons.F2Auth.Plugin.lastBearerTokens = BearerTokens(refreshTokenInfo.accessToken, refreshTokenInfo.refreshToken ?: "")
                    io.komune.im.commons.F2Auth.Plugin.lastBearerTokens
                }
            }
        }
